...
if (scenario !== 'timeout') {
    const acceptanceResult = scenario === 'success' || scenario === 'identity-mismatch' || scenario === 'pid-reuse-descendant' || scenario === 'cim-unavailable' || scenario === 'terminal-exit' ? 'success' : 'synthetic-failure';
...
if (scenario === 'terminal-exit') setTimeout(function () { process.exit(0); }, 900);
