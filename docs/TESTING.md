...
单元测试覆盖属性无回复、空值、桥接异常、监听器释放、日志脱敏与重复脱敏、外置插件读取旧配置/进程执行的封锁，以及 STRM Resolver 的判定、路径推导、命中和 fallback。修改 JS 已通过 node --check；PS 脚本由实际 PowerShell 5.1 构建与打包执行验证。
...
```powershell
powershell -NoProfile -ExecutionPolicy Bypass -File tools/test-runtime.ps1 -RuntimeName EmbyTheaterEnhanced-0.1.1-strm-mount-resolver-final3 -Visible -TestPipeline
```
...
```powershell
powershell -NoProfile -ExecutionPolicy Bypass -File tools/test-runtime.ps1 -RuntimeName EmbyTheaterEnhanced-0.1.1-strm-mount-resolver-final3 -Visible -TestPipeline -TestMount
```
...
|---|---|
| 输入哈希与解包 | 通过 |
| 诊断/外置入口/STRM Resolver 单元测试 | 17/17 通过 |
